...
void pushNode(char c)
{
	...
	if((newNode = malloc(sizeof(struct node))) == NULL) /*Allocate memory to the temp Node and check for any memory error*/
	{
...
void removeNode(char c) /*Increase the size of length*/
{
	...
	{
		struct node **tempNode = &head;
		while(*tempNode != NULL && (*tempNode)->data != c)
		{
...
void insertBefore(char A, char B)
{
	struct node **tempNode = &head;
	while(*tempNode != NULL && (*tempNode)->data != B)
	{
		tempNode = &(*tempNode)->next;
	}
	if(*tempNode != NULL && (*tempNode)->data == B)
	{
		struct node *newNode = NULL;
		if((newNode = malloc(sizeof(struct node))) == NULL) /*Allocate memory to the temp Node and check for any memory error*/
		{
...
void insertAfter(char A, char B)
{
	struct node **tempNode = &head;
	while(*tempNode != NULL && (*tempNode)->data != B)
	{
...
void processCommands(char commands[100][150],int count)
{
	...
	for(i = 0; i < count; i++)
	{
      ...
      pieces = strtok(NULL, " "); /*Store the second piece into pieces*/
		...
		{
			if(pieces[1] == '\n' && pieces[0] >= 'A' && pieces[0] <= 'Z') /*Check that the second pieces
			is between A-Z*/
			{
				value = pieces[0];
			}
			else if(pieces[3] == '\n' && pieces[0] >= 'A' && pieces[0] <= 'Z' && pieces[2] >= 'A' && pieces[2] <= 'Z')
			{
				value = pieces[0];
				value1 = pieces[2];
			}
		...
		if(strcmp("Append",command) == 0) /*Push command received*/
		{
			append(value);
		}
		else if(strcmp("InsertAfter",command) == 0) /*Push command received*/
		{
			insertAfter(value,value1);
		}
		else if(strcmp("InsertBefore",command) == 0) /*Push command received*/
		{
			insertBefore(value,value1);
		} 
		else if(strcmp("Push",command) == 0) /*Push command received*/
		{
			pushNode(value);
		}
